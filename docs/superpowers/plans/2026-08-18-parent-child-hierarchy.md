...
- [ ] **Step 1: 이미 있는 줄을 다듬는다**

Task 4에서 `[Unreleased] ### 추가`에 계층 표시 한 줄이 이미 들어갔다(커밋
`50f36e1`). 프로젝트 규칙상 **아직 태그가 안 나간 같은 릴리스 안의 기능은 줄을
새로 만들지 않고 원래 문장에 합쳐 고쳐 쓴다.**

그 줄에 Task 7(하위 추가)과 Task 8(자동 완료) 내용을 녹여 이렇게 다듬는다:

```markdown
- 사이드바에서 상위 작업 아래에 하위 작업이 한 칸 들여쓴 계층으로 보입니다 — 상위 작업 줄에는 하위 작업 진행률 막대와 "완료/전체"가 함께 나오고, 앞의 화살표로 접어둘 수 있습니다(접은 상태는 다음에 열 때도 유지됩니다). 작업 줄에 마우스를 올리면 나오는 + 로 하위 작업을 바로 추가할 수 있고, 하위 작업을 모두 끝내면 상위 작업도 저절로 완료됩니다
```

`### 추가` / `### 변경` / `### 수정` 세 가지만 쓰고, 항목이 없는 카테고리는
만들지 않는다. 이미 태그가 나간 버전 섹션은 건드리지 않는다.
...
- [ ] 수동: **다크 모드에서 부모 행이 자식과 구분돼 보이는가.** `.task.parent`의
      배경 `--panel-2`(#1c1f26)는 `--surface-task`(#1d2027)와 거의 같아
      (`app.css:26` 주석이 "nearly identical"이라 적어둔 값이다) 다크에서는
      접기 화살표와 진행 바로만 구분된다. 실물을 보고 부족하면 왼쪽 강조선 같은
      다른 단서를 더한다 — 라이트 모드는 문제없다.
